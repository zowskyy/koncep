...
describe("project status transitions", () => {
  it("allows the forward workflow", () => {
    ...
    expect(canTransitionProjectStatus("supported", "ceo_review")).toBe(true);
    expect(canTransitionProjectStatus("ceo_review", "completed")).toBe(true);
  });
  ...
  it("prevents transitions that skip stages", () => {
    ...
    expect(canTransitionProjectStatus("supported", "completed")).toBe(false);
  });
